...
public class Client {
  public static void main(String[] args) {
    ...
    csvRows.forEach(row -> all.add(new EmployeeCSVAdapter(row)));
    dbRows.forEach(row -> all.add(new EmployeeDBAdapter(row)));
    ldapRows.forEach(row -> all.add(new EmployeeLDAPAdapter(row)));
